...
namespace RPON
{
    public class ScreenCapture
    {
        ...
        public static Bitmap CaptureSelectedScreen(int screenNumber,bool captureMouse)
        {
                    ...
                    if (captureMouse)
                    {
                        ...
                        if (GetCursorInfo(out pci))
                        {
                            if (pci.flags == CURSOR_SHOWING)
                            {
                                var hdc = g.GetHdc();
                                // (IntPtr)65539
                                DrawIconEx(hdc, pci.ptScreenPos.x - bounds.X, pci.ptScreenPos.y - bounds.Y, pci.hCursor,
